    ...
    if current_number in all_numbers:
        ...
        if idx > 0:
            ...
            nav_buttons.append(
                InlineKeyboardButton(
                    f'⬅️ Урок {prev_num}',
                    callback_data=f'lesson_{prev_num}'
                )
            )
        if idx < len(all_numbers) - 1:
            ...
            nav_buttons.append(
                InlineKeyboardButton(
                    f'Урок {next_num} ➡️',
                    callback_data=f'lesson_{next_num}'
                )
            )
...
def ask_country() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(row_width=2)
    for country in ['Kazakhstan', 'Russia', 'Germany', 'Japan', 'France', 'USA']:
        kb.add(InlineKeyboardButton(country, callback_data=f'country_{country}'))
    kb.add(InlineKeyboardButton('⬅️ Главное меню', callback_data='menu_main'))
    return kb
